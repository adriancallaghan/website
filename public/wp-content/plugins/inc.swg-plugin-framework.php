...
/**
 * Framework class for all plugins. For WordPress 2.7+ only
...
class MaintenanceMode_SWGPluginFramework {
	...
	function Initialize($pluginInfo, $pluginDefaultOptions) {
 	...
 	/**
	 * Register plugin options page in the administration
	 */
	...
	function IniOrUpdateOptions() {
		...
		// Delete the options?
		if ( isset($_POST['delete-settings']) ) {
		...
		if ( !is_array($this->g_opt) || empty($this->g_opt) ) {
			// Options do not exist or have not yet been loaded so we set the default options
		...
		if ( isset($_POST['update-options']) ) {
			...
			foreach ($this->g_opt as $lpOptionName => $lpOptionValue) {
				if ( substr('$lpOptionName', -3) == '_lb' ) 
					// We want to use function "LinebreakToWhitespace"...
					$optionsToBeSaved[$lpOptionName] = $this->LinebreakToWhitespace($_POST[$lpOptionName]);
				else {
	...
	function GetGeneratedOptionsPage() {
		...
		<style type="text/css">
			...
			table#outer dl h3, table#outer dl h4 { font-size: 10pt; font-weight: bold; margin:0; padding: 4px 10px 4px 10px; background: #dfdfdf url(<?php echo $this->GetBase64IconURL('bg-header-gray.png'); ?>) repeat-x left top; }
		...
		<div class="submit">
			<?php wp_nonce_field($this->g_info['Name']) ?>
			<input type="submit" name="update-options" class="button-primary" value="<?php _e('Save Changes',$this->g_info['ShortName']) ?>" />
			<input type="submit" name="delete-settings" onclick='return confirm("<?php _e('Do you really want to delete/reset the plugin settings?',$this->g_info['ShortName']); ?>");' class="swg_warning" value="<?php _e('Delete/Reset Settings',$this->g_info['ShortName']) ?>" />
		</div>
