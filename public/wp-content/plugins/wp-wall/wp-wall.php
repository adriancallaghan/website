...
function WPWall_Credit() { 
   	
   	$options = WPWall_GetOptions();
   	
   	$links=array("WordPress Consulting", "WordPress Consultant", "WordPress Services", "WordPress Expert", "WordPress Developer", "WordPress SEO", "WordPress Professional", "WordPress Service");
   	
   	if (!($link=$options['credits']))   
   	{
   		$link=$links[rand(0, count($links)-1)];
   		$options['credits']=$link;
   		update_option('wp_wall', $options);
   	}
   	
		if (is_home())
			echo '<span style="font-size:9px; ">WP Wall by <a style="text-decoration:none" href="http://www.prelovac.com/vladimir/services/">'.$link.'</a></span>';
}
